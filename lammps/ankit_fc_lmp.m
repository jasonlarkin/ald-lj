...
LD.FC2.id = load('./update_list/FULLorder2.dat');
LD.FC3.id = load('./update_list/FULLorder3_2.dat');

LD.pos = load('./update_list/FULLpos.dat');

%--------------------------------------------------------------------------
%LD Input
%--------------------------------------------------------------------------
%define supercell integers
LD.Nx = 4;                LD.Ny = 4;                LD.Nz = 4;

%cell parameters
LD.cell = [1.5636 1.5636 1.5636 90 90 90 0 0 0 0 0 0];
%lattice vectors
LD.latvec =...
        [0.0 0.5 0.5;
		0.5 0.0 0.5;
...
%increment position for iFC,2
    LD.pos_tmp( LD.FC2.id(iFC,1) , LD.FC2.id(iFC,3) ) = ...
        LD.pos_tmp( LD.FC2.id(iFC,1) , LD.FC2.id(iFC,3) ) + LD.dx;
%calc changed force    
    LAMMPS.force = ankit_lmp_force(LD);
%change in force on iFC,1
    LD.FC2.phi(iFC) =...
        -(LAMMPS.force(LD.FC2.id(iFC,2) , LD.FC2.id(iFC,4) ) - ...
        LAMMPS.force0(LD.FC2.id(iFC,2) , LD.FC2.id(iFC,4) ) ) / LD.dx ;
%--------------------------------------------------------------------------
%pause  
%--------------------------------------------------------------------------  
end
